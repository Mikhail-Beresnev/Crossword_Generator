    ...
    def order_domain_values(self, var, assignment):
        ...
        for value in values:
            ...
            for neighbor in neighbors:
                nValues = self.domains[neighbor]
                x,y = self.crossword.overlaps[var,neighbor]
                for nValue in nValues:
                    if nValue  not in assignment.values():
                        if value[x] != nValue[y]:
                            count += 1
                        if value == nValue:
                            count += 1
            orderedList.append((value,count))
        orderedList = sorted(orderedList, key = lambda s: s[1])
        sortedList = []
        for i in range(len(orderedList)):
            sortedList.append(orderedList[i][0])
        return sortedList
        '''
